...
def check_rock(filled: set[tuple[int, int]], rock: list[tuple[int, int]], r: int, c: int):
    for coord in rock:
        if coord[0] + r <= 0: return False
        if not 0 <= coord[1] + c < 7: return False
        if (coord[0] + r, coord[1] + c) in filled: return False
    return True
...
def part1():
    ...
    for i in range(2022):
        ...
        while True:
            ...
            if not check_rock(filled, rock, r - 1, c): break
            r -= 1
        assert check_rock(filled, rock, r, c)
        for coord in rock:
